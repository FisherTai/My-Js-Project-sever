...
router.get("/instructor/:_instructor_id", (req, res) => {
  ...
  Course.find({ instructor: _instructor_id })
    ...
    .catch((e) => {
      res.status(500).send("Cannot find course the");
    });
});
...
router.patch("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      message:
        "Only the instructor of this course or web admin can edit for course",
    });
  }
});
...
router.delete("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      message:
        "Only the instructor of this course or web admin can delete for course",
    });
  }
});
